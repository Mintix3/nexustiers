...
// Players load from localStorage (if this browser has saved data from a
// previous session) so your edits persist across reloads/reopening the
// file. If nothing is saved yet, we fall back to players-data.js
// (PLAYERS_DATA) as the starting roster. Every add/edit/delete/import
// automatically re-saves to localStorage — see savePlayers() below.
...
function loadPlayers() {
  try {
    ...
    console.warn("Could not read saved player data, using defaults.", e);
  }
  return (typeof PLAYERS_DATA !== "undefined" ? PLAYERS_DATA : []).map(p =>
...
function moveInTier(playerId, cat, direction) {
